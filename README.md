...
- **Edge-detection methods** — fast, non-parametric TRGB estimates:
  ...
  - **GLOESS + Sobel** ([Persson et al. 2004](https://doi.org/10.1086/424934);
    [Madore & Freedman 2009](https://doi.org/10.1088/0004-637X/696/2/L98)) —
...
- **Photometric calibrations** (Rizzi et al. 2007, Fusco et al. 2012).
...
```julia
using TRGBDistances
using Optim          # loads OptimJL extension
using StableRNGs: StableRNG

# 1. Photometric calibration functions (from artificial star tests)
...
# 2. Generate (or load) dereddened I-band magnitudes
model_true = BrokenPowerLaw(24.0, 0.3, 0.2, 0.1)
mags = observe(StableRNG(1), model_true, 1000;
               err_func, complete_func, bias_func, upper_limit=2.0)

# 3. Quick non-parametric estimate with GLOESS+Sobel
result_gloess = gloess_trgb(mags; bandwidth=0.2, bin_width=0.1)
